...
function ricercaAnime(event)
{
    ...
    const cercata = document.getElementById("anime_text");
        ...
        $.ajax({
            ...
            dataType:"json",
            data:{
               valore: encodeURIComponent(encodeURIComponent(cercata.value))
            },
    ...
    in php è come se venisse tolto l'encoding.
    Se passo ?q=One%20Piece in ..\CercaController@cerca_anime  in $_POST['valore']
    viene salvato "One Piece". Quindi è come se si togliesse l'encode fatto. Il risultato è che la richiesta 
...
function gestisciPreferitiAnime(){
    
    const n1 = document.querySelector("#anime h2")
    const Titolo = n1.innerHTML;
    
    const n2 = document.querySelector("#anime h1");
    const original_title = n2.innerHTML;
    ...
    $.ajax({
        ...
        success: function(result){
            stampaCheckAnime(result);
        }
    });

}

function stampaCheckAnime(object){
   
    if(object == "Non trovato"){     
    const film = document.querySelector('#anime');
    const aggiungi = document.createElement('div');
    aggiungi.classList.add("messaggio");
    aggiungi.textContent="Clicca il più per aggiungerla alle anime da guardare";
    ...
    film.prepend(aggiungi);
    film.prepend(preferito);
    preferito.addEventListener("click", aggiungiAiPreferitiAnime);
        ...
        film.prepend(aggiungi);
        film.prepend(preferito);
        preferito.addEventListener("click", eliminaDaiPreferitiAnime);
    }
}


function aggiungiAiPreferitiAnime(){

    const n1 = document.querySelector("#anime h2")
    const Titolo = n1.innerHTML;
    
    const n2 = document.querySelector("#anime h1");
    const titoloOriginale = n2.innerHTML;
    
    const n3 = document.querySelector("#anime img");
    const immagine = n3.src;
   
    const n5 = document.querySelector("#anime p");
    const descrizione = n5.innerHTML;
    ...
    $.ajax({
        ...
        success: function(result){
            onConfermaAggiuntaAnime(result);
        }
    });
...
function eliminaDaiPreferitiAnime(){
    ...
    $.ajax({
        ...
        success: function(result){
            onConfermaCancellatoAnime(result);
        }
    });

}

function onConfermaCancellatoAnime(check){
    if(check == "Ok"){
        const checked = document.querySelector(".elimina");
        checked.style.backgroundImage="url(https://svgsilh.com/svg/152185.svg)";
        const messaggio = document.querySelector(".messaggio");
...
function onConfermaAggiuntaAnime(check){
